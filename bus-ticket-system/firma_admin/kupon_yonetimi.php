    ...
    if (empty($errors)) {
        try {
            if ($action === 'add') {
                ...
                $stmt = $pdo->prepare("INSERT INTO coupons (id, code, discount, company_id, usage_limit, expire_date, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'active', ?)");
                $stmt->execute([$new_coupon_id, $code, $discount, $company_id, $usage_limit, $expire_date, date('Y-m-d H:i:s')]);
                $_SESSION['success_message'] = 'Yeni kupon başarıyla oluşturuldu.';
...
if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['id'])) {
    if (empty($errors)) {
        $edit_mode = true;
        $stmt = $pdo->prepare("SELECT c.*, (SELECT COUNT(*) FROM user_coupons uc WHERE uc.coupon_id = c.id) as usage_count FROM coupons c WHERE c.id = ? AND c.company_id = ?");
...
$coupons_stmt = $pdo->prepare("SELECT c.*, (SELECT COUNT(*) FROM user_coupons uc WHERE uc.coupon_id = c.id) as usage_count FROM coupons c WHERE c.company_id = ? ORDER BY c.created_at DESC");
    ...
    <div class="row">
        <div class="col-md-3">
             <div class="list-group">
                ...
                <a href="<?php echo BASE_URL; ?>firma_admin/kupon_yonetimi.php" class="list-group-item list-group-item-action active">Kupon Yönetimi</a>
            </div>
        </div>
        ...
        <div class="col-md-9">
            <div class="card mb-4">
                <div class="card-header">
                    <h3><?php echo $edit_mode ? 'Firma Kuponunu Düzenle' : 'Yeni Firma Kuponu Oluştur'; ?></h3>
                </div>
                <div class="card-body">
                    <?php if (!empty($errors)): ?>
                    ...
                    if (!$page_error_message || ($edit_mode && $edit_coupon)) { ?>
                    <form action="<?php echo BASE_URL; ?>firma_admin/kupon_yonetimi.php<?php echo $edit_mode ? '?action=edit&id='.$edit_coupon['id'] : ''; ?>" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3"><label for="discount" class="form-label">İndirim Oranı (%)</label><input type="number" class="form-control" name="discount" value="<?php echo htmlspecialchars($_POST['discount'] ?? $edit_coupon['discount'] ?? ''); ?>" placeholder="Örn: 15" <?php if($edit_mode && $edit_coupon_used) echo 'readonly'; ?> required></div>
                        </div>
                        <div class="row">
                        ...
                        <?php endif; ?>
                        <?php if ($edit_mode): ?><a href="<?php echo BASE_URL; ?>firma_admin/kupon_yonetimi.php" class="btn btn-secondary">İptal</a><?php endif; ?>
                    </form>
                    <?php } ?>
                </div>
            </div>

            <h3>Firma Kuponları</h3><hr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="<?php echo BASE_URL; ?>firma_admin/kupon_yonetimi.php?action=edit&id=<?php echo $coupon['id']; ?>" class="btn btn-sm btn-warning <?php if($is_used) echo 'disabled'; ?>">Düzenle</a>
                            
                            <?php ?>
                            <?php if ($coupon['status'] === 'active'): ?>
                                <a href="<?php echo BASE_URL; ?>firma_admin/kupon_sil.php?id=<?php echo $coupon['id']; ?>" class="btn btn-sm btn-secondary" onclick="return confirm('Bu kuponu PASİF yapmak istediğinizden emin misiniz?');">Pasif Yap</a>
                            <?php else: ?>
                                <a href="<?php echo BASE_URL; ?>firma_admin/kupon_sil.php?id=<?php echo $coupon['id']; ?>" class="btn btn-sm btn-success" onclick="return confirm('Bu kuponu AKTİF yapmak istediğinizden emin misiniz?');">Aktif Yap</a>
                            <?php endif; ?>
                            <?php ?>
                    ...
                    <?php endforeach; ?>
                    <?php if (empty($coupons_data)): ?><tr><td colspan="6" class="text-center">Henüz oluşturulmuş kupon bulunmamaktadır.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
